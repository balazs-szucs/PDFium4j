...
int pdfium4j_get_custom_xmp(FPDF_DOCUMENT doc, const char* ns_uri, const char* key, char* buf, int buf_len) {
    ...
    pugi::xpath_node_set nodes = xdoc.select_nodes(("//*[namespace-uri()='" + std::string(ns_uri) + "' and local-name()='" + std::string(key) + "']").c_str());
...
int pdfium4j_get_custom_xmp_bag(FPDF_DOCUMENT doc, const char* ns_uri, const char* key, char* buf, int buf_len) {
    ...
    int xmp_len = pdfium4j_get_xmp_metadata(doc, nullptr, 0);
    if (xmp_len <= 1) {
        printf("pdfium4j_get_custom_xmp_bag: no XMP found\n");
        return 0;
    }
    ...
    if (!xdoc.load_buffer(raw.data(), xmp_len)) {
        printf("pdfium4j_get_custom_xmp_bag: failed to parse XMP\n");
        return 0;
    }
    
    std::string xpath = "//*[namespace-uri()='" + std::string(ns_uri) + "' and local-name()='" + std::string(key) + "']//*[local-name()='li' and namespace-uri()='http://www.w3.org/1999/02/22-rdf-syntax-ns#']";
    pugi::xpath_node_set nodes = xdoc.select_nodes(xpath.c_str());
    printf("pdfium4j_get_custom_xmp_bag: found %zu nodes for %s\n", nodes.size(), xpath.c_str());
...
int pdfium4j_set_custom_xmp(FPDF_DOCUMENT doc, const char* ns_uri, const char* prefix, const char* key, const char* value) {
    ...
    printf("pdfium4j_set_xmp: writing XMP:\n%s\n", out.c_str());
