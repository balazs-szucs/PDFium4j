...
public final class ThumbnailBindings {
  ...
  private static MethodHandle findOptional(FunctionDescriptor desc) {
    java.util.Optional<java.lang.foreign.MemorySegment> addr =
        LOOKUP.find("FPDFPage_GetThumbnailAsBitmap");
    return addr.map(memorySegment -> LINKER.downcallHandle(memorySegment, desc)).orElse(null);
  }

  private static final StableValue<MethodHandle> FPDFPage_GetThumbnailAsBitmap_SV =
      StableValue.of();
